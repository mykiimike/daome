...
class daome {
	...
	tableUpdate() {
		...
		self.pageNumber = Math.floor(self.pollingData.length/self.pageTableElement);
		self.pageNumber += self.pollingData.length%self.pageTableElement ? 1 : 0;

		/* check for overflow */
		if(self.pageTable >= self.pageNumber)
			self.pageTable = 0;
		...
		if(dataTo > self.pollingData.length)
			dataTo = self.pollingData.length;

		if(self.pagination) {
			// prev
			if(self.pageTable == 0)
				self.pagination.prev.setAttribute("disabled", "disabled");
			else
				self.pagination.prev.removeAttribute("disabled");

			// next
			if(self.pageTable >= self.pageNumber-1)
				self.pagination.next.setAttribute("disabled", "disabled");
			else
				self.pagination.next.removeAttribute("disabled");

			// current page
			self.pagination.current.innerHTML = self.pageTable+1;

			// number of elements
			self.pagination.right.innerHTML = "Elements from "+(dataFrom+1)+" to "+dataTo+" of "+self.pollingData.length+" entries";

			// number of pages
			self.pagination.pages.innerHTML = self.pageNumber+" pages";
		}
	...
	pageUpdate() {
		...
		var element = this.paginationElement;
		var output = document.getElementById(element);
		this.cleanElement(output);
		this.pagination = {};

		var root = this.createElement("table", {
			class: "daome-page-root"
		});
		var row = this.createElement("tr", {
			class: "daome-page-row"
		});
		root.appendChild(row);

    var col1 = this.createElement("td", {
			class: "daome-page-table-left"
    });
    var col2 = this.createElement("td", {
			class: "daome-page-table-right"
    });
    row.appendChild(col1);
    row.appendChild(col2);

    /* col2 */
    self.pagination.right = col2;
    ...
    /* prev */
    var th = this.createElement("td", {});
    var a = genButton("fa-backward");
